...
],auth.verifyUser,auth.verifyCustomer,profile.addProfileCustomer)
...
],auth.verifyUser,auth.verifyCompany,profile.addProfileCompany)


router.get('/profile1',auth.verifyUser,profile.showProfileCompany);
...
router.put('/profile/editProfileCompany',[
    check('company',"Name must be entered").not().isEmpty(),
    check('foundedin',"Date must be entered").not().isEmpty(),
],auth.verifyUser,auth.verifyCompany,profile.editProfileCompany)


router.put('/profile/editProfileCustomer',[
    check('projects',"Project/s must be entered").not().isEmpty(),
    check('experience',"Experience must be entered").not().isEmpty(),
],auth.verifyUser,auth.verifyCustomer,profile.editProfileCustomer)
